...
const VertoScreen = () => {
  ...
  const [vertoParams, setVertoParams] = useState({
    webSocket: {
      // url: 'wss://NGLOSDCFS227.telvida.com:8082',
      // url: 'wss://172.16.2.27:8082',
      url: 'wss://meet.thehavennation.org:8082',
      // login: '1008@cantina.freeswitch.org',
      // login: '1003@NGLOSDCFS227.telvida.com',
      // login: '1003@NGLOSDCFS227.telvida.com',
      login: '1008@meet.thehavennation.org',
      password: 'serricorn'
    },
    deviceParams: {
      useMic: 'any',
  ...
  useEffect(() => {
    console.log('[VertoScreen - useEffect[vertoParams]] vertoParams has changed!');
  ...
  const onLoginHandler = (login: string, password: string, url: string) => {
    if(!login ||!password) {
      ...
      console.log('I shoud put login process here');
  ...
  const onCameraSwitchHandler = () => {
    console.log('[VertoScreen - onCameraSwitchHandler] cameraFacing:', cameraFacing);
